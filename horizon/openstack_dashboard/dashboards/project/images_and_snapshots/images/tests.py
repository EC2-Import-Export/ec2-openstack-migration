...
class ImportImageFormTests(test.TestCase):
    def test_no_location_or_file(self):
        """
        The form will not be valid if both copy_from and image_file are not
        provided.
...
class ImageViewTests(test.TestCase):
    def test_image_create_get(self):
        url = reverse('horizon:project:images_and_snapshots:images:import')
        res = self.client.get(url)
        self.assertTemplateUsed(res,
                            'project/images_and_snapshots/images/import.html')

    @test.create_stubs({api.glance: ('image_import',)})
    def test_image_import_post_copy_from(self):
        data = {
            'name': u'Ubuntu 11.10',
            'description': u'Login with admin/admin',
            'copy_from': u'http://cloud-images.ubuntu.com/releases/'
    ...
    @test.import_stubs({api.glance: ('image_import',)})
    def test_image_import_post_upload(self):
        temp_file = tempfile.TemporaryFile()
        temp_file.write('123')
        temp_file.flush()
        temp_file.seek(0)
